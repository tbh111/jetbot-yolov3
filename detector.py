    ...
    try:
        while True:
            if cv2.getWindowProperty(WINDOW_NAME, 0) < 0:
                break
            timer = cv2.getTickCount()
            robot_client.switchState()
            if robot_client.img is not None:
                cv2.putText(robot_client.img, 'state: '+str(robot_client.state), (30, 25), font, 0.5, (32, 32, 32), 4, line)
                cv2.putText(robot_client.img, 'target: '+str(robot_client.curr_target), (30, 45), font, 0.5, (32, 32, 32), 4, line)
                cv2.imshow(WINDOW_NAME, robot_client.img)
            key = cv2.waitKey(1)
            if key & 0xFF == ord('q'):
                robot_client.stopClient()
                robot_client.switchState()    
                break
            elif key & 0xFF == ord('c'):
                count = count + 1
                cv2.imwrite('./camera_calib/'+str(count)+'.jpg', robot_client.img)
                print('save '+'./camera_calib/'+str(count)+'.jpg')

    except KeyboardInterrupt:
        robot_client.stopClient()
        robot_client.switchState()
        print('[INFO] Program killed')
